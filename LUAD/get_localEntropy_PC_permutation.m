...
for l=1:psize(2)
    for na=1:total_node_num
        ...
        for i=1:e
            curr_pcc(i)=abs(corr(tempcontrol(edges_list(i,1),D)',...
                tempcontrol(edges_list(i,2),D)'));
        end
        ...
        for s=1:patients_num(l)    % psize(3)
            for i=1:e
                temp_add_onecase1=[tempcontrol(edges_list(i,1),D),...
                    reshape(tempcase(edges_list(i,1),l,s),1,1)];
                temp_add_onecase2=[tempcontrol(edges_list(i,2),D),...
...
save weighted_delta_entropy_permutation.mat weighted_delta_entropy;
load('weighted_delta_entropy_permutation.mat');
close all;
